...
class FbTransService
{
    ...
    public function createTransOrder($user, $post)
    {
        ...
        //收款方式验证
        // $post['pay_method']
        $pay_arr = explode(',', $chu_order->pay_method);
        if (!in_array($post['pay_method'], $pay_arr)) {
            throw new Exception(sprintf('订单不支持该支付/收款方式%s', $post['pay_method']));
        }
...
//            $order_info['tpath2'] = $user->relationship;
    ...
    public function countDown($order_info): int
    {
        ...
        switch ($order_info->status) {
            case 1:
                $down_time = self::getCountDown(2);//自动取消时间
                ...
                break;
            case 2:
                $down_time = self::getCountDown(1);//自动确认时间
    ...
    public static function getOverRate($uid, $type)
    {
        if ($type == 1) {
            $model = FbSell::query();
        } else {
        ...
        $totalNum = $model->where(['uid' => $uid])->count();

        $overNum = $model->where(['uid' => $uid,'status'=>2])->count();

        if ($totalNum > 0) {
            $rate = bcMath($overNum, $totalNum, '/', 2);
        }
        return [
            'over_rate' => $rate ?? 0,
            'over_num'  => $overNum ?? 0
    ...
    public function shopApply($user)
    {
        ...
        $assetService->writeBalanceLog($user->id, $apply->id, $wid, UserAsset::ACCOUNT_LEGAL, -$amount,
            UserMoneyLog::BUSINESS_TYPE_FB_SHOP, '成为商家-冻结金额');
